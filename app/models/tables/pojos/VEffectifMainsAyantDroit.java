/*
 * This file is generated by jOOQ.
*/
package models.tables.pojos;


import java.io.Serializable;
import java.sql.Timestamp;

import javax.annotation.Generated;


/**
 * This class is generated by jOOQ.
 */
@Generated(
    value = {
        "http://www.jooq.org",
        "jOOQ version:3.10.1"
    },
    comments = "This class is generated by jOOQ"
)
@SuppressWarnings({ "all", "unchecked", "rawtypes" })
public class VEffectifMainsAyantDroit implements Serializable {

    private static final long serialVersionUID = -109396733;

    private Long      id;
    private String    nomAy;
    private Timestamp dateNaiss;
    private Integer   age;
    private Long      adherent;
    private String    nomAdherent;
    private String    matricule;
    private Timestamp whenDone;
    private String    whoDone;
    private Boolean   onDeleted;
    private String    picture;
    private String    lien;
    private String    genre;

    public VEffectifMainsAyantDroit() {}

    public VEffectifMainsAyantDroit(VEffectifMainsAyantDroit value) {
        this.id = value.id;
        this.nomAy = value.nomAy;
        this.dateNaiss = value.dateNaiss;
        this.age = value.age;
        this.adherent = value.adherent;
        this.nomAdherent = value.nomAdherent;
        this.matricule = value.matricule;
        this.whenDone = value.whenDone;
        this.whoDone = value.whoDone;
        this.onDeleted = value.onDeleted;
        this.picture = value.picture;
        this.lien = value.lien;
        this.genre = value.genre;
    }

    public VEffectifMainsAyantDroit(
        Long      id,
        String    nomAy,
        Timestamp dateNaiss,
        Integer   age,
        Long      adherent,
        String    nomAdherent,
        String    matricule,
        Timestamp whenDone,
        String    whoDone,
        Boolean   onDeleted,
        String    picture,
        String    lien,
        String    genre
    ) {
        this.id = id;
        this.nomAy = nomAy;
        this.dateNaiss = dateNaiss;
        this.age = age;
        this.adherent = adherent;
        this.nomAdherent = nomAdherent;
        this.matricule = matricule;
        this.whenDone = whenDone;
        this.whoDone = whoDone;
        this.onDeleted = onDeleted;
        this.picture = picture;
        this.lien = lien;
        this.genre = genre;
    }

    public Long getId() {
        return this.id;
    }

    public void setId(Long id) {
        this.id = id;
    }

    public String getNomAy() {
        return this.nomAy;
    }

    public void setNomAy(String nomAy) {
        this.nomAy = nomAy;
    }

    public Timestamp getDateNaiss() {
        return this.dateNaiss;
    }

    public void setDateNaiss(Timestamp dateNaiss) {
        this.dateNaiss = dateNaiss;
    }

    public Integer getAge() {
        return this.age;
    }

    public void setAge(Integer age) {
        this.age = age;
    }

    public Long getAdherent() {
        return this.adherent;
    }

    public void setAdherent(Long adherent) {
        this.adherent = adherent;
    }

    public String getNomAdherent() {
        return this.nomAdherent;
    }

    public void setNomAdherent(String nomAdherent) {
        this.nomAdherent = nomAdherent;
    }

    public String getMatricule() {
        return this.matricule;
    }

    public void setMatricule(String matricule) {
        this.matricule = matricule;
    }

    public Timestamp getWhenDone() {
        return this.whenDone;
    }

    public void setWhenDone(Timestamp whenDone) {
        this.whenDone = whenDone;
    }

    public String getWhoDone() {
        return this.whoDone;
    }

    public void setWhoDone(String whoDone) {
        this.whoDone = whoDone;
    }

    public Boolean getOnDeleted() {
        return this.onDeleted;
    }

    public void setOnDeleted(Boolean onDeleted) {
        this.onDeleted = onDeleted;
    }

    public String getPicture() {
        return this.picture;
    }

    public void setPicture(String picture) {
        this.picture = picture;
    }

    public String getLien() {
        return this.lien;
    }

    public void setLien(String lien) {
        this.lien = lien;
    }

    public String getGenre() {
        return this.genre;
    }

    public void setGenre(String genre) {
        this.genre = genre;
    }

    @Override
    public String toString() {
        StringBuilder sb = new StringBuilder("VEffectifMainsAyantDroit (");

        sb.append(id);
        sb.append(", ").append(nomAy);
        sb.append(", ").append(dateNaiss);
        sb.append(", ").append(age);
        sb.append(", ").append(adherent);
        sb.append(", ").append(nomAdherent);
        sb.append(", ").append(matricule);
        sb.append(", ").append(whenDone);
        sb.append(", ").append(whoDone);
        sb.append(", ").append(onDeleted);
        sb.append(", ").append(picture);
        sb.append(", ").append(lien);
        sb.append(", ").append(genre);

        sb.append(")");
        return sb.toString();
    }
}
